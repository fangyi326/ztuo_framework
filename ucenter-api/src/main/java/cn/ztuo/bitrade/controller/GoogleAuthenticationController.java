...
public class GoogleAuthenticationController extends BaseController{
    ...
    public MessageResult jcgoogle(String codes, @SessionAttribute(SESSION_MEMBER) AuthMember user,String smsCode) {
        // enter the code shown on device. Edit this and run it fast before the
        // code expires!
...
//        }
        ...
        if (!smsCode.equals(redisUtil.get(SysConstant.RESET_GOOGLE_CODE_PREFIX + member.getMobilePhone()))) {
            return error(sourceService.getMessage("VERIFICATION_CODE_INCORRECT"));
        } else {
            redisUtil.delete(SysConstant.RESET_GOOGLE_CODE_PREFIX + member.getMobilePhone());
        }
        long code = Long.parseLong(codes);
        long t = System.currentTimeMillis();
        GoogleAuthenticatorUtil ga = new GoogleAuthenticatorUtil();
